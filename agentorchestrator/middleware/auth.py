...
class AuthMiddleware:
    ...
    async def validate_api_key(self, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            key_exists = (
                self.redis.hexists("api_keys", api_key) or 
                self.redis.exists(f"apikey:{api_key}")
            )
            if not key_exists:
            ...
            if key_data:
                try:
                    ...
                    if not isinstance(parsed_data, dict) or "key" not in parsed_data:
                        self.logger.error("Invalid key data format in traditional store")
                        return None
    ...
    async def check_auth(self, request: Request) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(f"Processing request {request.method} {request.url.path} with key: {api_key[:5]}...")
            ...
            if api_key_data.get("key") != api_key:
                self.logger.warning(f"Key mismatch: stored key does not match provided key")
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Invalid API key",
                )
                
            self.logger.debug(f"Successfully authenticated request with key: {api_key[:5]}...")
            return api_key_data
    ...
    async def send_error_response(self, send: Callable, status_code: int, detail: str) -> None:
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            api_key = request.headers.get(self.config.api_key_header)
            if not api_key:
                self.logger.warning(f"Missing API key for {request.method} {request.url.path}")
                response = {
                    "success": False,
                    "error": {
            ...
            try:
                ...
                api_key_data = await self.validate_api_key(api_key)
                if not api_key_data:
                    self.logger.warning(f"Invalid API key {api_key[:5]}... for {request.method} {request.url.path}")
                    response = {
                        "success": False,
                        "error": {
                            "code": status.HTTP_401_UNAUTHORIZED,
