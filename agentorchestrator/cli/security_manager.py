...
@click.option(
    ...
    type=int,
    default=0,
    help="Days until expiration (0 = no expiration)",
)
@click.option(
    "--ip-whitelist", "-i", multiple=True, help="IP addresses allowed to use this key"
)
@click.option(
    "--redis-url", "-u", default=None, help="Redis URL (defaults to REDIS_URL env var)"
)
def generate_api_key(
    role: str, name: str, expires: int, ip_whitelist: list[str], redis_url: str | None
):
    """
    ...
    async def _generate_key():
        try:
            ...
            click.echo(
                f"IP Whitelist: {', '.join(ip_whitelist) if ip_whitelist else 'None (all IPs allowed)'}"
            )
            click.echo(
...
def list_api_keys(redis_url: str | None):
    """
        ...
        try:
            ...
            for key in keys:
                ...
                if metadata_str:
                    ...
                    click.echo(
                        f"  IP Whitelist: {', '.join(metadata.get('ip_whitelist', [])) or 'None'}"
                    )
                    click.echo("")
...
def revoke_api_key(key_id: str, redis_url: str | None):
    """
...
def assign_permission(role: str, permission: str, redis_url: str | None):
    """
...
def list_roles(redis_url: str | None):
    """
...
@click.option(
    ...
    "-k",
    default=None,
    help="Encryption key (defaults to ENCRYPTION_KEY env var)",
)
def encrypt_value(value: str, key: str | None):
    ...
    if not encryption_key:
        click.echo(
            "Error: Encryption key not provided and ENCRYPTION_KEY environment variable not set",
            err=True,
        )
        sys.exit(1)
...
def decrypt_value(value: str, key: str | None):
    """
...
def generate_encryption_key_file(filename: str):
    ...
    try:
        ...
        click.echo(
            f"To use this key, set ENCRYPTION_KEY={key} in your environment variables"
        )
        click.echo(
